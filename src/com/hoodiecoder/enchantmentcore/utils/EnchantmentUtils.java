	...
	for (Map<Enchantment, Integer> map : maps) {
		...
		for (Entry<Enchantment, Integer> entry : map.entrySet()) {
			for (Entry<Enchantment, Integer> combinedEntry : combined.entrySet()) {
				if (allowIllegal ? false : combinedEntry.getKey().conflictsWith(entry.getKey()))
					continue initialEntryLoop;
			}
			if (!(itemToEnchant.getItemMeta() instanceof EnchantmentStorageMeta) && (allowIllegal ? false : !entry.getKey().canEnchantItem(itemToEnchant)))
				continue initialEntryLoop;
